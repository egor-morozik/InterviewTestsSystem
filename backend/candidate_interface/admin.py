...
from .models import Answer, Candidate, Invitation, QuestionFeedback
...
class InvitationAdmin(admin.ModelAdmin):
    ...
    list_filter = ("interview_type", "test_template", "sent", "completed")
    search_fields = (
        "candidate__email",
        "test_template__name",
    )
    ...
    def view_answers(self, obj):
        if obj.answers.exists():
            url = (
                reverse("admin:candidate_interface_answer_changelist")
                + f"?invitation__id__exact={obj.id}"
            )
    ...
    def send_single_invitation(self, request, invitation_id):
        ...
        if send_test_invitation_email(invitation):
            self.message_user(
                request, f"Письмо отправлено на {invitation.candidate.email}"
            )
        else:
        ...
        return HttpResponseRedirect(
            request.META.get(
                "HTTP_REFERER", "admin:candidate_interface_invitation_changelist"
            )
        )
    ...
    def get_queryset(self, request):
        ...
        if request.user.is_tech_lead and not request.user.is_hr:
            return qs.filter(
                assigned_tech_lead=request.user, interview_type="technical"
            )
        return qs
...
class AnswerAdmin(admin.ModelAdmin):
    ...
    list_filter = ("invitation__test_template", "invitation__candidate", "score")
    search_fields = ("invitation__candidate__email", "question__text", "response")
    readonly_fields = ("invitation", "question", "response", "score")

    def question_truncated(self, obj):
        return (
            obj.question.text[:60] + "..."
            if len(obj.question.text) > 60
    ...
    def correct_answer_truncated(self, obj):
        return (
            obj.question.correct_answer[:60] + "..."
            if len(obj.question.correct_answer) > 60
            else obj.question.correct_answer
        )
